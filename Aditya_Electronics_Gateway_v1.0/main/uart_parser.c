...
/* ── RX task ─────────────────────────────────────────────────── */
typedef enum {
    ST_IDLE=0, ST_SW_DATA, ST_SW_END, ST_VT_DATA, ST_VT_END
} rx_state_t;
...
static void uart_rx_task(void *arg)
{
    ...
    uint8_t    sw_data[4] = {0};
    uint8_t    vt_data[7] = {0};   /* was 6 - NUC now also sends bat_pct */
    uint8_t    sw_didx = 0u;   /* separate index for switch  */
    uint8_t    vt_didx = 0u;   /* separate index for voltage */
